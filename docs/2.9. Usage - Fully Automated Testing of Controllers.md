...
- `action` - no answers given, return `null` once the required actions have been completed
...
The test suite begins with a `pre_tests_message` and ends with a `post_tests_message`. These should be treated as action questions and return an `null` `answer_response` to begin/end the actual tests.
